...
class DDIMSampler(object):
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        ori_feature=kwargs['ori_feature']
        
        #grad = self.get_classifier_guidance(pred_x0, x_target, classifier, classifier_scale)
        grad = self.get_classifier_guidance(pred_x0, x_target,ori_feature, classifier, classifier_scale)
        #print("grad:",grad)
        #exit()
    ...
    #         #print(f"img shape: {img.shape}, x_target shape: {x_target.shape}")
    #         for k, v in classifier.items():
    #             resize = torch.nn.AdaptiveAvgPool2d((224, 224)) if k == 'ResNet152' else ( \
    #             torch.nn.AdaptiveAvgPool2d((112, 112)) if k != 'FaceNet' else torch.nn.AdaptiveAvgPool2d((160, 160)))
    ...
    def get_classifier_guidance(self, x_0, x_target, x_orig, classifier, classifier_scale, lambda_reg=0.1):
        from torch.nn import functional as F
        with torch.enable_grad():
            B = x_0.shape[0]
            similarity_score = 0
            ...
            x_in = x_0.detach().requires_grad_(True)
            img = self.model.decode_first_stage(x_in)
            x_orig=self.model.decode_first_stage(x_orig)
            
            # 同时提取参考特征和原图特征
            for k, v in classifier.items():
                resize = torch.nn.AdaptiveAvgPool2d((224, 224)) if k == 'ResNet152' else (
                    torch.nn.AdaptiveAvgPool2d((112, 112)) if k != 'FaceNet' 
                    else torch.nn.AdaptiveAvgPool2d((160, 160))
                )
                
                # 生成图像特征
                feature_gen = v(resize(img)).reshape(B, -1)
                # 参考图像特征
                feature_ref = v(resize(x_target)).reshape(B, -1)
                # 原图特征 (新增)
                feature_orig = v(resize(x_orig)).reshape(B, -1)
                
                # 计算余弦相似度
                similarity_score += F.cosine_similarity(feature_gen, feature_ref).sum() / B
                # 计算L2正则项 (新增)
                similarity_score += F.cosine_similarity(feature_gen, feature_orig).sum() / B
                
                i += 1
            
            # 总损失 = 相似度项 + 正则项
            total_score = (similarity_score / i) + lambda_reg * (reg_score / i)
            
            # 梯度计算需同时考虑两项
            return torch.autograd.grad(total_score, x_in)[0] * classifier_scale
